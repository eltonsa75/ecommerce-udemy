...
class CouponController {
  ...
  async store ({ request, response }) {
     ...
     * 3 - clients - e products - pode ser utilizados somente em produtos e clientes específicos
     ...
     */
     ...
     try {
        ...
        const { users, product } = request.only(['users', 'product'])
      ...
      return response.status(400).send({
        message: 'Não foi possível criar o coupon no momento!'
      })
     }
  }
  ...
  async update ({ params: { id }, request, response }) {
    ...
    try {
      ...
      const { users, products } = request.only(['users', 'product'])
  ...
  async destroy ({ params: { id }, request, response }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({
        message: 'Não foi possível deletar este cupon no momento!'
      })
    }
